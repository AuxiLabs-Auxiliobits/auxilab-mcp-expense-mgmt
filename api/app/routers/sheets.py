...
def scan_receipt(
    ...
    Deliberately a *sync* endpoint: the Document Intelligence call is blocking I/O, so FastAPI
    must run it in the threadpool. As `async def` it would block the event loop, serializing the
    per-line-item scans the review view fires on render — later items then exceed the client
    timeout and surface as "couldn't read the receipt" on all but the first line item."""
    sheet = sheet_service.get_sheet_or_404(session, sheet_id)
    rbac_scope.assert_can_view_sheet(principal, sheet)
    item = sheet_service.get_line_item_or_404(session, sheet, line_item_id)
