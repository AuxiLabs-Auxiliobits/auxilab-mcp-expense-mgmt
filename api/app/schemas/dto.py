...
class SheetUpdate(BaseModel):
    """Edit a draft/returned sheet's title, period, or employee note."""
...
class SheetOut(BaseModel):
    ...
    employee_note: str | None = None  # employee's note before resubmission (shown to manager)
    ...
    # Policy clauses the AI cited when auto-approving (APPROVED path only). Shown to Finance
    # for audit so they can verify the AI's reasoning and override if needed.
    ...
    # How the sheet reached its current terminal/decided state — drives the UI clarity banner
    # in the finance review drawer so Finance can immediately see who/what decided and why.
    #   "AI_AUTO_APPROVED"       — LLM auto-approved, no human overrode it yet
    #   "FINANCE_APPROVED"       — Finance human-approved (manual review path, no prior AI decision)
    #   "FINANCE_OVERRIDE"       — Finance overrode an AI auto-approval (approve → override)
    #   "FINANCE_REJECTED"       — Finance rejected (manual)
    #   "FINANCE_OVERRIDE_REJECTED" — Finance rejected what the AI had auto-approved
...
class NotificationPrefs(BaseModel):
    """The known notification toggles owned by the Settings page. Unknown keys are dropped
    (``extra="ignore"``) and values are coerced to bool, so the persisted store can't be
    polluted with arbitrary data (one key per toggle, nothing else)."""

    model_config = {"extra": "ignore", "populate_by_name": True}

    status_changes: bool | None = Field(default=None, alias="statusChanges")
    info_requests: bool | None = Field(default=None, alias="infoRequests")
    payment_confirmations: bool | None = Field(default=None, alias="paymentConfirmations")


class PreferencesIn(BaseModel):
    """User settings (notification toggles) owned by the Settings page. Validated against the
    known keys so the JSON column stays a clean, predictable boolean map."""
...
class PolicyMatchEvidence(BaseModel):
    """One retrieved policy chunk that the AI approver considered for a sheet."""
...
class PolicyCheckOut(BaseModel):
    """On-demand AI policy evaluation of a sheet (read-only; never changes status).
    Shows Finance exactly which policy points the sheet was checked against and the AI's
    verdict + cited clauses, for any sheet regardless of when it was decided (SCOPING §6.3)."""

    decision: FinanceDecision
    confidence: float
    cited_clauses: list[str] = Field(default_factory=list)
    reason_detail: str | None = None
    evidence: list[PolicyMatchEvidence] = Field(default_factory=list)
    policy_found: bool = False  # was any agency policy retrieved from the RAG index?
    llm_used: bool = False  # did the LLM produce the verdict (vs. offline fallback)?
...
class PolicyEventOut(BaseModel):
    """A single audit event for a policy version, returned by the ingestion log endpoint."""
...
class PolicyChunkOut(BaseModel):
    """One indexed chunk from Azure AI Search, returned by the chunk-verification endpoint."""
...
class PolicyIndexSummaryOut(BaseModel):
    """Per-version chunk count summary for one agency — returned by the index-summary endpoint."""

    agency_id: str
    versions: list[dict]  # [{version, chunks, has_vectors}]
    total_chunks: int
