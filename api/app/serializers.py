...
def _assemble_sheet(
    ...
    # Compute decision_method and auto_approval_reasons for decided/terminal states.
    # decision_method drives the clarity banner in the Finance review drawer so Finance can
    # immediately see whether this was AI-decided, human-approved, or a human override.
    if sheet.status in (
        SheetStatus.FINANCE_APPROVED, SheetStatus.APPROVED, SheetStatus.PAID,
        SheetStatus.FINANCE_REJECTED, SheetStatus.REJECTED,
    ):
        all_dec = list(session.exec(
            select(Decision)
            .where(Decision.sheet_id == sheet.id)
            .order_by(Decision.timestamp.asc())
        ).all())
        llm_dec = next((d for d in all_dec if d.actor_id == "llm-approver"), None)
        had_finance = any(d.actor_role in ("finance", "admin") for d in all_dec)

        # Populate auto_approval_reasons ONLY for AI-auto-approved sheets, not for sheets
        # where a human Finance user made the decision (even if the AI ran first).
        if sheet.status == SheetStatus.FINANCE_APPROVED and llm_dec and llm_dec.cited_clauses:
            try:
                clauses = json.loads(llm_dec.cited_clauses)
                if clauses:
                    out.auto_approval_reasons = [str(c) for c in clauses]
            except (ValueError, TypeError):
                pass

        # Decision method tag for UI clarity.
        if sheet.status == SheetStatus.FINANCE_APPROVED:
            out.decision_method = "AI_AUTO_APPROVED"
        elif sheet.status in (SheetStatus.APPROVED, SheetStatus.PAID):
            if llm_dec and had_finance:
                out.decision_method = "FINANCE_OVERRIDE"
            elif had_finance:
                out.decision_method = "FINANCE_APPROVED"
        elif sheet.status == SheetStatus.FINANCE_REJECTED:
            out.decision_method = "FINANCE_REJECTED"
        elif sheet.status == SheetStatus.REJECTED:
            out.decision_method = "FINANCE_OVERRIDE_REJECTED" if llm_dec else "FINANCE_REJECTED"
...
def sheet_to_out(
    ...
    return _assemble_sheet(session, sheet, items, by_item, name_of, agency_of, policy)
...
def sheets_to_out(
    ...
    return [
        _assemble_sheet(session, s, items_by_sheet.get(s.id, []), atts_by_item, name_of, agency_of, policy)
        for s in sheets
    ]
