...
def _notify_employee_outcome(session: Session, sheet: ExpenseSheet, approved: bool | None) -> None:
    ...
    notification_service.notify(
        ...
        href=f"/employee/sheets/{sheet.id}", category="statusChanges",
    )
...
def run_offline_approver(
    session: Session, sheet: ExpenseSheet, items: list[LineItem]
) -> ExpenseSheet:
    """AI Finance Approver (SCOPING §6.3). Checks the sheet against the agency's indexed
    policy documents via RAG + LLM, comparing line item descriptions, merchants, amounts,
    and receipt scan data against the policy. Auto-approves with cited policy clauses when
    all items comply; routes to Finance manual-review with specific violation reasons when
    they don't. Falls back to the receipt-scan flag check when Azure Search / Foundry are
    not configured. Reuses `apply_llm_decision` so the decision trail is identical."""
    from app.config import settings as _settings  # noqa: PLC0415
    from app.services.policy_compliance_service import check_sheet_against_policy  # noqa: PLC0415

    decision, confidence, cited, _reason = check_sheet_against_policy(
        sheet, items, _settings
    )
    model_version = (
        "policy-rag-approver@v1" if _settings.azure_search_enabled else "offline-approver@local"
    )
    return apply_llm_decision(
        session, sheet,
        decision=decision,
        model_version=model_version,
        policy_version=sheet.policy_version_used or "baseline-v1",
        cited_clauses=cited,
        confidence=confidence,
    )


def process_pending_finance(session: Session, principal: Principal) -> dict[str, int]:
    """Run the offline approver on every sheet stuck in IN_FINANCE_REVIEW within the caller's
    scope. Lets Finance clear sheets that were handed off before the worker/offline approver
    existed (or while Service Bus was down). Agency-scoped for Managers; org-wide for Finance.
    Returns counts of {processed, approved, routed}."""
    stmt = select(ExpenseSheet).where(ExpenseSheet.status == SheetStatus.IN_FINANCE_REVIEW)
    ...
    for sheet in pending:
        ...
        result = run_offline_approver(session, sheet, items)
        if result.status is SheetStatus.FINANCE_APPROVED:
            approved += 1
        elif result.status is SheetStatus.FINANCE_MANUAL_REVIEW:
            routed += 1
    return {"processed": len(pending), "approved": approved, "routed": routed}
