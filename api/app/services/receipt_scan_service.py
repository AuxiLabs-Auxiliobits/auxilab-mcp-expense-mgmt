...
def scan_receipt(blob_uri: str, entered_amount: Decimal | None, settings: Settings) -> ReceiptScanOut:
    ...
    try:
        if settings.doc_intel_endpoint:
            ...
            except Exception as exc:  # noqa: BLE001 — auth/endpoint error → degrade, don't fail
                # e.g. 401 (bad/expired key or AAD token). Fall through to the text parser so a
                # misconfigured Doc-Intelligence doesn't flag every receipt; surface it in detail.
                structured = None
                _doc_intel_error = f"Document Intelligence unavailable ({exc})."
            else:
                _doc_intel_error = None
        ...
        if not text.strip():
            return _flagged_unavailable(
                entered_amount,
                _doc_intel_error
                or "No text extracted. Configure Azure Document Intelligence for image/PDF OCR.",
            )
        try:
            parsed = parse_receipt(text, _llm(settings))
        except Exception as llm_exc:  # noqa: BLE001 — LLM cred/endpoint error → degrade
            # e.g. 401 from Azure OpenAI when the Foundry key is wrong/missing. Don't surface
            # the raw SDK error; just flag for human review with a clean reason.
            detail = (
                _doc_intel_error
                or f"AI receipt parser unavailable — check APP_FOUNDRY_API_KEY / APP_FOUNDRY_ENDPOINT ({type(llm_exc).__name__})."
            )
            return _flagged_unavailable(entered_amount, detail)
...
def _scan_with_doc_intelligence(data: bytes, settings: Settings):
    ...
    # Prefer the API key for local dev (no managed identity / az login). Falling straight to
    # DefaultAzureCredential here is what produced "scan failed: DefaultAzureCredential failed
    # to retrieve a token" on local receipts — and that spurious failure flags the line item
    # for review. Mirror the worker's extractor: key when set, else managed identity.
