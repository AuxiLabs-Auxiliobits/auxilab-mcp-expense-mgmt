...
from app.services import audit_service, intake_service, malware_scan_service, notification_service
...
def update_sheet(
    ...
    if data.employee_note is not None:
        sheet.employee_note = data.employee_note or None  # blank string → NULL
...
def update_line_item(
    ...
    # Editing an item invalidates its prior verdicts (SCOPING §5.1, §6.1): the old policy/
    # receipt-scan result was computed against the previous values, so we clear those stale
    # flags here. Without this, a returned item keeps its old POLICY_FAIL badge even after the
    # employee fixes it — which keeps the Resubmit button disabled (errorCount > 0), a catch-22
    # where the only way to clear the flag (resubmit) is itself blocked by the flag. The
    # authoritative re-check runs fresh at resubmission (run_intake).
    if changed:
        item.policy_status = None
        item.policy_clause_ref = None
...
def attach_receipt(
    ...
    *, filename: str, data: bytes, file_type: str,
    ...
    # Malware scan before magic-byte check so an executable disguised as a document
    # surfaces as "Malware detected" rather than the less-clear extension mismatch error.
    if malware_scan_service.scan_bytes(data, filename) == malware_scan_service.ScanStatus.INFECTED:
        raise HTTPException(
            status.HTTP_422_UNPROCESSABLE_ENTITY,
...
def submit_sheet(
    ...
    # Hard block: only malware/infected receipts (manager cannot safely open them).
    # Everything else — policy issues, reconciliation mismatch, duplicate risk — reaches
    # the manager as POLICY_UNCERTAIN flags; the manager decides, not the system.
    ...
    for item in items:
        ...
        if intake_service.intake_hard_fails(check):
            malware_blocked = True
            item.policy_status = LineItemStatus.POLICY_FAIL
            malware_reasons.append(f"{item.description or item.id}: infected receipt detected")
        elif intake_service.intake_has_warnings(check):
            # Soft flag — manager sees it, system doesn't auto-return.
            item.policy_status = LineItemStatus.POLICY_UNCERTAIN
            warning = intake_service.intake_warning_reason(check)
            item.needs_human_review = True
            item.review_reason = warning
            intake_warnings.append(f"{item.description or item.id}: {warning}")
        else:
            item.policy_status = LineItemStatus.POLICY_PASS

    title = sheet.title or "Expense sheet"
    if malware_blocked:
        # Only genuine security issue still auto-returns (manager can't view infected files).
        sheet.status = SheetStatus.RETURNED_TO_EMPLOYEE
        combined_reason = "; ".join(malware_reasons)
        audit_service.record(
        ...
        session.add(Decision(
            sheet_id=sheet.id,
            actor_id="intake-gate",
            actor_role=Role.AGENT,
            action="INTAKE_RETURNED",
            reason=combined_reason,
        ))
        notification_service.notify(
            session, recipient_id=sheet.employee_id, kind="warning", icon="error",
            title="Sheet returned — infected receipt",
            body=f'"{title}" was returned: an attached receipt failed the malware scan. '
                 "Please remove and re-upload a clean file.",
            href=f"/employee/sheets/{sheet.id}", entity=f"expense_sheet:{sheet.id}",
            agency_id=sheet.agency_id, category="statusChanges",
        )
    else:
        # Always advance to manager review — even when there are soft flags.
        ...
        if intake_warnings:
            # Write a Decision row so the manager's Decision Trail shows the flagged items.
            session.add(Decision(
                sheet_id=sheet.id,
                actor_id="intake-gate",
                actor_role=Role.AGENT,
                action="INTAKE_FLAGGED",
                reason="; ".join(intake_warnings),
            ))
        flagged_note = f" ({len(intake_warnings)} item(s) flagged for your attention)" if intake_warnings else ""
        notification_service.notify_role_in_agency(
            ...
            body=f'"{title}" is awaiting your review.{flagged_note}',
...
def _reset_for_resubmission(sheet: ExpenseSheet, items: list[LineItem]) -> None:
    ...
    manager/finance verdicts are reset (SCOPING §5.1). The employee_note is preserved
    across the version bump so the manager sees the employee's explanation."""
    sheet.version += 1
    sheet.finance_decision = None
    sheet.finance_decided_by = None
    for item in items:
        ...
        item.policy_status = None
        item.policy_clause_ref = None
        # Clear the prior receipt-scan flag too — intake re-runs and re-flags from the
        # current values, so a stale mismatch flag never carries into the new version.
        item.needs_human_review = False
        item.review_reason = None
...
def manager_action(
    ...
    session.add(item)
    # new_status is already "MANAGER_APPROVED"/"MANAGER_REJECTED"; only INFO_REQUESTED needs the
    # manager prefix (avoids the old double-prefixed "MANAGER_MANAGER_APPROVED" audit action).
    action = (
        "MANAGER_INFO_REQUESTED"
        if new_status is LineItemStatus.INFO_REQUESTED
        else str(new_status)
    )
    audit_service.record(
...
def _maybe_advance_after_manager(
    ...
    if {LineItemStatus.MANAGER_REJECTED, LineItemStatus.INFO_REQUESTED} & statuses:
        ...
        notification_service.notify(
            ...
            href=f"/employee/sheets/{sheet.id}", entity=f"expense_sheet:{sheet.id}",
            agency_id=sheet.agency_id, category="infoRequests",
        )
        ...
        # Hand off to Line 3. In production the sheet is enqueued to Service Bus and the
        # workers package (LangGraph approver) consumes it. With no Service Bus configured
        # (local/dev), run the approver inline so the sheet actually reaches a finance
        # decision instead of waiting forever in IN_FINANCE_REVIEW.
        from app import messaging  # local import avoids an import cycle

        if not messaging._use_azure():
            from app.services import finance_service  # local import avoids an import cycle

            finance_service.run_offline_approver(session, sheet, items)
            return  # the approver set the final status, updated_at, and audit trail
    sheet.updated_at = utcnow()
    session.add(sheet)
