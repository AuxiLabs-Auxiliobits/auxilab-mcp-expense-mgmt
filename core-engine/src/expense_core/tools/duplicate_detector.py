...
def _classify_match(
    ...
    if c.receipt_datetime is not None and h.receipt_datetime is not None:
        ...
        # Both items have no date. Same total without a date to confirm it's the same
        # transaction (e.g. two different $50 meals) → near-match (MEDIUM) for manager
        # to judge, not an auto-return-worthy exact duplicate.
        return "NEAR_MATCH_WINDOW"
