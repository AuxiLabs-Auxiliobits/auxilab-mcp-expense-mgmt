...
def intake_passes(check: ClaimCheck) -> bool:
    """A line item clears intake when policy passes, there's no high duplicate risk, every
    receipt scanned clean, and the receipt reconciliation didn't hard-fail."""
    policy_ok = (check.policy_result or {}).get("status") == PolicyCheckStatus.PASS.value
    dup_risk = (check.duplicate_result or {}).get("risk")
    dup_ok = dup_risk in (None, "none", "low")
    receipt = check.receipt_result or {}
    scan_ok = (receipt.get("scan") or {}).get("status") == ScanStatus.CLEAN.value
    reconcile_ok = (receipt.get("reconcile") or {}).get("status") != "fail"
    return policy_ok and dup_ok and scan_ok and reconcile_ok
...
def _reconcile_status(item: LineItem, attachments: list[Attachment], scan: dict) -> dict:
    # Deterministic tax sanity on the entered figures (no OCR needed) — hard errors.
    tax = item.tax
    total = item.receipt_total
    if tax is not None and tax < Decimal("0"):
        return {"status": "fail", "detail": "Tax is negative."}
    if tax is not None and total is not None and tax > total:
        return {"status": "fail", "detail": "Tax exceeds the receipt total."}
