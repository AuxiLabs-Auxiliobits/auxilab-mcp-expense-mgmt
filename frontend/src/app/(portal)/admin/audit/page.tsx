...
export default function AdminAuditPage() {
  const { isLoading } = useCurrentUser("admin");
  ...
  return (
    <PageContainer>
      ...
      {/* The activity feed is already role-scoped by the backend token, so this page just
          renders the shared table once the current user is available. */}
      {!isLoading && <ActivityTable />}
    </PageContainer>
  );
}
