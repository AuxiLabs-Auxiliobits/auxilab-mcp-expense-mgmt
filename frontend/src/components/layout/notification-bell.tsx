...
export function NotificationBell() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useAllNotifications(sessionRole);
...
function IconBtn({
  ...
  return (
    <button
      ...
      disabled={spinning}
      className={cn(
        "rounded-md p-1.5 text-on-surface-variant transition-colors hover:bg-surface-container-high hover:text-on-surface focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-secondary disabled:opacity-70",
        small && "p-1",
      )}
    >
      <Icon name={name} className={cn(small ? "text-[16px]" : "text-[18px]", spinning && "animate-spin")} />
    </button>
  );
}
