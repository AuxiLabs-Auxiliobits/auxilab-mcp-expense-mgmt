...
/** Extracted text of a stored policy doc — lazy (only when the viewer opens). */
export const usePolicyDocumentContent = (policyId: string | undefined, enabled = true) =>
  useQuery({
    queryKey: ["policy-content", policyId ?? ""],
    queryFn: () => api.getPolicyDocumentContent(policyId as string),
    enabled: !!policyId && enabled,
  });

/** All chunks indexed in Azure AI Search for an agency — for chunk verification UI. */
export const useAgencyChunks = (agencyId: string | undefined, enabled = true) =>
  useQuery({
    queryKey: ["agency-chunks", agencyId ?? ""],
    queryFn: () => api.getAgencyChunks(agencyId as string),
    enabled: !!agencyId && enabled,
  });

/** Per-version chunk count summary from Azure AI Search — for index verification. */
export const useAgencyIndexSummary = (agencyId: string | undefined) =>
  useQuery({
    queryKey: ["agency-index-summary", agencyId ?? ""],
    queryFn: () => api.getAgencyIndexSummary(agencyId as string),
    enabled: !!agencyId,
  });

/** Ingestion audit trail for one policy version — fetched lazily when the log panel opens. */
export const usePolicyIngestLog = (
  agencyId: string | undefined,
  policyId: string | undefined,
  enabled = true,
) =>
  useQuery({
    queryKey: ["policy-ingest-log", agencyId ?? "", policyId ?? ""],
    queryFn: () => api.getPolicyIngestLog(agencyId as string, policyId as string),
    enabled: !!agencyId && !!policyId && enabled,
  });

/** On-demand RAG policy evaluation for a sheet (matched policy points + verdict). */
export const useSheetPolicyCheck = (sheetId: string | undefined, enabled = true) =>
  useQuery({
    queryKey: ["sheet-policy-check", sheetId ?? ""],
    queryFn: () => api.getSheetPolicyCheck(sheetId as string),
...
/** Run the AI finance approver on sheets stuck in finance review (clears the backlog). */
export function useProcessPendingApprover() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: api.processPendingApprover,
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: queryKeys.routedSheets });
      qc.invalidateQueries({ queryKey: queryKeys.allSheets });
      qc.invalidateQueries({ queryKey: queryKeys.financeKpis });
      qc.invalidateQueries({ queryKey: queryKeys.auditLog });
      qc.invalidateQueries({ queryKey: ["activity-log"] });
    },
  });
...
export function useDeletePolicy() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: api.deletePolicyDocument,
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: queryKeys.policyDocuments });
      qc.invalidateQueries({ queryKey: queryKeys.auditLog });
    },
...
// Per-item mutations take the notification id. Invalidating the role's notifications prefix
// refreshes both the bell (useNotifications) and the center (useAllNotifications).
