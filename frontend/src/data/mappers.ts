  ...
  return {
    ...
    citedClause:
      Array.isArray(cited) && cited.length
        ...
        : undefined,
    employeeNote: (r.employee_note ?? r.employeeNote) as string | undefined,
    autoApprovalReasons: Array.isArray(r.auto_approval_reasons)
      ? (r.auto_approval_reasons as string[])
      : undefined,
    decisionMethod: (r.decision_method as string | undefined) ?? undefined,
    lineItems,
  };
}
...
const SEVERITY_BY_ACTION = (action: string): AuditLogEntry["severity"] => {
  ...
  if (a.includes("REJECT") || a.includes("ESCALATION") || a.includes("DELETE") || a.includes("DEACTIVAT")) return "error";
  if (a.includes("DISCARD") || a.includes("WITHDRAW") || a.includes("RECALL") || a.includes("RETURN") || a.includes("ROUTE") || a.includes("INFO")) return "warning";
  if (a.includes("APPROVE") || a.includes("CREATE") || a.includes("PUBLISH") || a.includes("SUBMIT") || a.includes("DRAFT")) return "success";
  return "info";
};
