...
export interface User {
  ...
  createdAt?: string; // UTC ISO — when the account was created
}
...
export interface PolicyIngestEvent {
  id: string;
  action: string;
...
export interface ExpenseSheet {
  ...
  employeeNote?: string;
  /** Per-line-item policy checks that passed at auto-approval time — shown to Finance for audit. */
  autoApprovalReasons?: string[];
  /**
   * How the sheet reached its decided state — drives the decision clarity banner.
   * "AI_AUTO_APPROVED" | "FINANCE_APPROVED" | "FINANCE_OVERRIDE" |
   * "FINANCE_REJECTED" | "FINANCE_OVERRIDE_REJECTED"
   */
