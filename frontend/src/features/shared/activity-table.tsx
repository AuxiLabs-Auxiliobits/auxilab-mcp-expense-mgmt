...
/** Parse backend entity string (e.g. "expense_sheet:abc123") into a label + optional href. */
function parseEntity(
  ...
  const href = type === "expense_sheet" && id ? sheetHref(id) : null;
  return { label, href };
}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-body-sm">
              ...
              <tbody className="divide-y divide-outline-variant">
                {items.map((e) => {
                  const ent = parseEntity(e.entity, sheetHref);
                  return (
                    <tr key={e.id} className="align-top transition-colors hover:bg-surface-bright">
                      ...
                      <td className="px-4 py-2.5 text-on-surface">
                        {e.summary}
                        {ent && (
                          <span className="ml-1 font-mono text-label-sm text-on-surface-variant">· {ent.label}</span>
                        )}
                      </td>
                    </tr>
                  );
