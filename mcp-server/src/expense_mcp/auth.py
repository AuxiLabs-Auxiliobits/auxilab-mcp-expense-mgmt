...
# Module-level fallback so `login` persists across stdio tool calls (ContextVars are
# isolated per async task; the embedding-host concurrent case still wins via _token_var).
...
def get_token() -> str | None:
    return _token_var.get() or _global_token or config.bootstrap_token
